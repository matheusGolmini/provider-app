...
const ProfileEditEmail = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        setIsLoading(true);
        await ProviderService.upadatePerson({ email: values.email });
        setIsLoading(false);
        ...
        <View style={{ ...styles.container, backgroundColor: "#fff" }}>
          <View
            style={{
              ...styles.action,
              borderBottomColor:
                formik.touched.email && formik.errors.email ? "red" : "#F2F2F2",
            }}
          >
            <Icon
              name="email"
              size={30}
              color={
                formik.touched.email && formik.errors.email ? "red" : "#605C99"
              }
            />
            <TextInput
              placeholder="E-mail"
